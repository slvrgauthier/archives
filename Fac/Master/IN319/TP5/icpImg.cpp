...
void computeCorrespondences(MESH& mesh, const CImg<float> &dist,const float l)
{
    ...
    for(unsigned int i=0;i<nbpoints;i++)
    {
        ...
        float p[3], norm[3], pc[3];
        mesh.getPoint(p,i);
        mesh.getNormal(norm, i);

        pc[0]= p[0]+l*valeur*norm[0];
        pc[1]= p[1]+l*valeur*norm[1];
        pc[2]= p[2]+l*valeur*norm[2];
...
CImg<float> computeDistance(const CImg<unsigned char> &sourceProf,const CImg<unsigned char> &targetProf,const Metric metric)
{
    ...
    else if(choix ==2) // on choisie ncc
    {
...
/// A COMPLETER(fini)
    ...
    for( int i=0 ; i<N; i++)
    {
        ...
        for(int j=Ni-1; j>=0 ; j--)
        {
            ...
            p[1]=-norm[1]*l+p[1];
            p[2]=-norm[2]*l+p[2];
            valNi = img.getValue(p,interpolationType);
            prof(j,i)= valNi;
        ...
        for(int j=0; j<No ; j++)
        {
            ...
            p[1]=norm[1]*l+p[1];
            p[2]=norm[2]*l+p[2];
            valNo = img.getValue(p,interpolationType);
            prof(Ni+1+j,i)= valNo;
	...
	for(unsigned int i=0;i<mesh.getNbPoints();i++) {
        ...
        for(unsigned int j=0;j<3;j++) {
            for(unsigned int k=0;k<3;k++) {
                Q[j][k] += (p[j]-c[j])*(p[k]-c[k]);
                K[j][k] += (cp[j]-c0[j])*(p[k]-c[k]);
            }
        }
    }
