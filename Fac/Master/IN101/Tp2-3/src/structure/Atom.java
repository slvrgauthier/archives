...
public class Atom
{
	...
	public Atom(Atom a, Substitution s) { //constructeur instanciant l'atome avec s
	...
	public boolean unifiable(Atom a) {
		if(this.equalsA(a))
			return true;
		if(predicate.equals(a.getPredicate()) && this.getArity() == a.getArity()) {
			for(int i=0; i<this.getArity();i++) {
				Term t1 = this.getArgI(i), t2 = a.getArgI(i);
				if(!t1.equalsT(t2)) {
					if(t2.isVariable()) { //t1 constant ou variable, même unification
						CoupleTerms c = new CoupleTerms(t2,t1);
						Substitution s = new Substitution();
						s.add(c);
						return this.unifiable(new Atom(a,s)); //atome instancié
					}
					else if(t1.isVariable()) { //t2 constant
						CoupleTerms c = new CoupleTerms(t1,t2);
						Substitution s = new Substitution();
						s.add(c);
						return a.unifiable(new Atom(this,s)); //atome instancié
					}
					else { //t1 et t2 constants non-égaux
						return false;
					}
				}
			}
		}
		//prédicats ou arités non-égaux
		return false;
	}
	/**
