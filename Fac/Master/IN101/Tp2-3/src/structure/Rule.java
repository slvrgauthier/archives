...
public class Rule
{
	...
	public Rule(Rule r, Substitution s) { //instanciation de règle
		hypothesis = new ArrayList<Atom>();
		terms = new ArrayList<Term>();
		for(Atom a : r.getHypothesis()) {
			Atom a2 = new Atom(a, s);
			boolean add = true;
			for(int i=0;i<hypothesis.size();i++)
				if(hypothesis.get(i).equalsA(a2))
					add = false;
			if(add)
				hypothesis.add(a2);
   			for (int i = 0; i < a2.getArity(); i ++)
   				a2.setArgI(i,searchOrAddTerm(a2.getArgI(i)));
		}
		conclusion = new Atom(r.getConclusion(), s);
	}
